...
def run_linear_benchmark_with_batch(
    basis_file: str,
    alpha: float = 1.0,
    ...
    dummy_xyz = jnp.ones(shape=(batch_size, num_atoms, 3), dtype=dtype)

    # V, f = model.get_energy_and_forces(dummy_xyz)
    @jax.jit
    def run_eval(packed_xyz: jax.Array):
        return jax.vmap(model.get_energy_and_forces)(packed_xyz)

    V, f = run_eval(dummy_xyz)
    ...
    return {
        ...
        "run_per_sample": run_time / batch_size,
        "throughput": batch_size / run_time,
    }
...
if __name__ == "__main__":
    ...
    res = run_linear_benchmark_with_batch(
